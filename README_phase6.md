...
**LINE Developers コンソールから Messaging API チャネルを直接作る方法は廃止された。**
いまは「LINE公式アカウントを作る → Messaging API を有効にすると、チャネルが
自動生成される」という順序になる。ネット上の古い手順と食い違うので注意。

1. [LINE Developers](https://developers.line.biz/console/) に
   **「LINEアカウント」でログイン**する。メールアドレス(ビジネスアカウント)でも
   ログインできるが、3-2 の宛先IDの確認で困る。
2. 開発者アカウントを作る(**開発者名**と**メールアドレス**)。どちらも管理用で、
   通知を受け取る人には見えない。あとから変更できる。
3. **プロバイダー**を作る。チャネルを束ねる開発者側の単位。
4. プロバイダーの「チャネル設定」タブで **「Messaging API」のタイル**を選ぶ。
   チャネルが直接できるのではなく、**LINE公式アカウントの作成フォーム**に移る。
   - **アカウント名**: ここで付けた名前が、**受け取る人の友だちリストに出る**。
   - **ビジネスマネージャーの組織**: 初回は「組織を作成」。組織名は空欄でよい
     (アカウント名と同じ名前になる)。プロバイダーとは別物で、個人利用では
     とくに使わない。
   - **アカウント認証**: 「あとで認証を行う」でよい。認証済アカウントは法人・
     個人事業主向けで、**個人利用は対象外**。未認証でも Messaging API の push は
     制限なく使え、無料枠も同じ。
5. 作成後、LINE Developers のそのプロバイダー配下にチャネルが並ぶ。
   並んでいなければ [LINE Official Account Manager](https://manager.line.biz/) で
   **設定 → Messaging API → 「Messaging APIを利用する」**に進み、プロバイダー選択で
   **既存のプロバイダーを選ぶ**(新規作成しない)。**プロバイダーは後から変更できない**。
6. チャネルを開き、**「Messaging API設定」タブの最下部**にある
   **「チャネルアクセストークン(長期)」**を **発行**する。これが
   `LINE_CHANNEL_ACCESS_TOKEN`。
   - **有効期限はない**が、**再発行すると古いトークンは即座に無効になる**
     (チャネルにつき1件)。誤って再発行したら Secrets も更新すること。
   - **持っている人は誰でもこのアカウントとして送信できる**。Secrets 以外の場所に
     置かない。
7. こちらから送るだけの静かなアカウントにするなら、**LINE Official Account Manager**
   の **設定 → 応答設定**で「応答メッセージ」をオフにする(LINE Developers 側の画面
   ではない)。push だけなら Webhook も不要。

### 3-2. 受け取る側(宛先ID)を調べる

**送りたい相手には、先に公式アカウントを友だち追加してもらう**必要がある。
友だちでない相手には push できない(403 が返る)。自分の分も忘れずに:
チャネルの「Messaging API設定」タブに出る **QRコード**をスマホで読むと追加できる。
